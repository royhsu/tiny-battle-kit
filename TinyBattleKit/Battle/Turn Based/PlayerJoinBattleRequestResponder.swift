...
// MARK: - PlayerJoinBattleResponse

public struct PlayerJoinBattleResponse: BattleResponse {
    
    // MARK: Property
    
    public let updatedRecord: TurnBasedBattleRecord
...
public struct PlayerJoinBattleRequestResponder {
    ...
    public func respond(to request: BattleRequest) -> Promise<PlayerJoinBattleResponse> {
        ...
        return Promise(in: .main) { fulfull, reject, _ in
            ...
            fulfull(
                PlayerJoinBattleResponse(updatedRecord: updatedRecord)
            )
